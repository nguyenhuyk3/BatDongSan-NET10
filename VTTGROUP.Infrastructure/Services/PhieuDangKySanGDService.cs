...
{
    public class PhieuDangKySanGDService
    {
        private readonly IDbContextFactory<AppDbContext> _factory;
        private readonly string _connectionString;
        private readonly ILogger<PhieuDangKySanGDService> _logger;
        private readonly IConfiguration _config;
        private readonly ICurrentUserService _currentUser;
        private readonly INotificationSender _notifier;
        private readonly IBaseService _baseService;
        private readonly NavigationManager _nav;
        public PhieuDangKySanGDService(IDbContextFactory<AppDbContext> factory, ILogger<PhieuDangKySanGDService> logger, IConfiguration config,
            ICurrentUserService currentUser, INotificationSender notifier, IBaseService baseService, NavigationManager nav)
        {
            _factory = factory;
            _logger = logger;
            _config = config;
       ...
       string? maDuAn, string? maSanGG, int page, int pageSize, string? qSearch, string fromDate, string toDate, string? trangThai)
        {
        ...
        //              join chc in _context.DaDanhMucDuAnCauHinhChungs on da2.MaDuAn equals chc.MaDuAn into dtCauHinhCung
        //              from chc2 in dtCauHinhCung.DefaultIfEmpty()
        ...
        //                  GiaBanTheoCSTT = 0,
        //                  ISMoBanCoGia = chc2.IsMoBanCoGia ?? false
        //              }).FirstOrDefaultAsync();
        //        entity.NguoiLap = await _currentUser.GetThongTinNguoiLapAsync();
        //        entity.MaPhieu = await SinhMaPhieuTuDongAsync("PDK-", _context, 5);
        //        entity.SanGiaoDich = entity.NguoiLap.MaSanGiaoDich;
        ...
        //        entity.GiaBan = await _context.BhGioHangCanHos.Where(d => d.MaPhieuGioHang == id && d.MaCanHo == maCanHo).Select(d => d.GiaBanSauPhanBo).FirstOrDefaultAsync() ?? 0;
        ...
        public async Task<ResultModel> GetByGioHangIdAsync(string id, string maCanHo, SoDoCanHoModel soDo)
        {
            ...
            try
            {
                ...
                // 1) Check tình trạng sản phẩm (đã bán => điều hướng + return sớm)
                var trangThai = await _baseService.CanShowDangKyAsync(soDo.MaDuAn ?? string.Empty, maCanHo, id);
                //await _baseService.HienTrangKinhDoanhSanPham(soDo.MaDuAn, maCanHo);
                if (!trangThai.Status)
                {
                    _nav.NavigateTo("/sanpham-dadangky");
                ...
                // 2) Query chính: Giỏ hàng + dự án + sàn + đợt + MaLoaiThietKe (gom luôn)
                var entity = await (
                    ...
                    from dmb2 in dmbGroup.DefaultIfEmpty()
                        ...
                        // ✅ join theo maCanHo (left join để không lỗi nếu thiếu)
                    join sp in context.DaDanhMucSanPhams.AsNoTracking()
                        on maCanHo equals sp.MaSanPham into spGroup
                ...
                // 3) Enrich
                entity.NguoiLap = await _currentUser.GetThongTinNguoiLapAsync();
                entity.MaPhieu = await SinhMaPhieuTuDongAsync("PDK-", context, 5);
                entity.SanGiaoDich = entity.NguoiLap.MaSanGiaoDich;
                entity.TenSanGiaoDich = entity.NguoiLap.TenSanGiaoDich;
        ...
        //                  TyLeChietKhau = cs2.TyLeChietKhau ?? 0,
        //              }).FirstOrDefaultAsync();
        //        if (entity != null)
        //        {
        ...
        //        {
        //            MaPhieu = await SinhMaPhieuTuDongAsync("PDK-", _context, 5),
        ...
        //        record.SanGiaoDich = record.NguoiLap;
               ...
               */
                ...
                if (flagKHT == null)
                {
                    ...
                    if (khTam != null)
                    {
                        ...
                        {
                            MaKhachHang = kh.MaKhachHang,
                            IdlanDieuChinh = await TaoMaKhachHangMoiAsync(kh.MaKhachHang, _context),
